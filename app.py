...
def about():
    ...
    <p>I created this open-source app in a few hours on a Sunday morning while reviewing the latest academic papers during my Ph.D. studies. The app generates AI-driven summaries focused on quantitative findings in the abstractss from a number of top journals in the field of Communication, using their publicly available RSS feeds.</p>
    <p>This application uses a large language model (LLM) API—currently Gemini 2.0 Flash, chosen for its high rate limits and strong performance among free LLMs—to generate concise summaries of recent academic papers. It is designed to help researchers stay up to date with the latest developments, particularly in quantitative research.</p>
    <p>Why the focus on quantitative research? Because its results are typically presented in structured formats—such as statistical analyses, tables, and models—allowing for quick access to key empirical insights without losing essential meaning. In contrast, qualitative studies rely more on nuanced interpretations and contextual depth, which require full engagement with the text to fully appreciate their insights.</p>
